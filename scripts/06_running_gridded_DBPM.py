...
if __name__ == '__main__':
    ...
    depth = xr.open_zarr(glob(os.path.join(
        base_folder, 'gridded', model_res, '*obsclim_deptho_*'))[0])['deptho']
    ...
    #Spinup data is loaded if init_time is None or if the init_time year is 
    #less than 1960
    if init_time is None or init_yr < 1959:
        # Loading effort
        effort = xr.open_zarr(glob(os.path.join(
            gridded_folder, 'effort_spinup*'))[0])['effort']
    #Spinup data plus obsclim are loaded if init_time is 1960
    elif init_yr >= 1959 and init_yr < 1961:
        # Loading effort
        effort = xr.open_mfdataset(glob(os.path.join(
            gridded_folder, 'effort_*')), engine = 'zarr')['effort']
    #Obsclim data loaded from 1961 onwards:
    else:
        # Loading effort
        effort = xr.open_zarr(glob(os.path.join(
            gridded_folder, 'effort_[0-9]*'))[0])['effort']
    ...
    for t in range(0, len(ds_dynamic.time)):
        ...
        try:
            ...
            ds_dynamic['effort'] = xr.where(
                ds_dynamic.time == ds_dynamic.time[t+1], eff_short.values, 
                ds_dynamic['effort'])
            #Remove variables not needed
            del dt_eff, fn
        except:
            dt = pd.to_datetime(ds_dyn.time.values).strftime('%Y-%m')
            eff_short = xr.open_dataarray(glob(os.path.join(
                out_folder, f'effort*{dt}*'))[0])
            ds_dynamic['effort'] = xr.where(
                ds_dynamic.time == ds_dynamic.time[t], eff_short.values, 
                ds_dynamic['effort'])
        
        ds_init = uf.gridded_sizemodel(
            gridded_params, ds_fixed, ds_init, ds_dyn, region = region, 
            model_res = model_res, out_folder = out_folder)
